...
object MongoHelpers {
  ...
  object MongoError {
    ...
    class MongoErrorHandler[F[_]: MonadError[?[_], MongoError]]
      ...
      private val handler: MongoError => F[Response[F]] = {
        case InvalidObjectId(id)         => BadRequest(s"BSON Id $id not valid".asJson)
        case ObjectNotFound(id)          => BadRequest(s"Not found id $id".asJson)
        case DuplicateUniqueKeyError(id) => Conflict(s"id $id already exists!".asJson)
        case ObjectWithPropertyNotFound(propName, propValue) =>
          NotFound(s"Property not found: $propName with value $propValue".asJson)
      }
  ...
  def lift[F[_], A](future: => Future[A])(
      implicit F: LiftIO[F]
  ): F[A] = F.liftIO(IO.fromFuture(IO(future)))
  ...
  def create[F[_]: LiftIO, O](
      ...
      id: BSONObjectID,
      obj: O
  )(implicit F: Sync[F], ev: BSONDocumentWriter[WithId[O]]): F[Unit] =
    lift(mongoCollection.insert(WithId(id.stringify, obj)).void).handleErrorWith {
      case de: DatabaseException if de.code == Some(DuplicateUniqueKeyErrorCode) =>
        F.raiseError(DuplicateUniqueKeyError(de.message))
    }

  def update[F[_]: LiftIO, U, O](
      mongoCollection: BSONCollection,
      id: BSONObjectID,
      updateData: U
  )(implicit
    ...
    lift(
      ...
      case ce: DefaultBSONCommandError if ce.code == Some(DuplicateUniqueKeyErrorCode) =>
        F.raiseError(DuplicateUniqueKeyError(ce.errmsg.getOrElse("")))
    }
  ...
  def remove[F[_]: LiftIO](
    ...
    lift(
      ...
      if (wr.n > 0) F.unit else F.raiseError(ObjectNotFound(id.stringify))
    }
  ...
  def removeByProperty[F[_]: LiftIO](
    ...
    lift(
      ...
      if (wr.n > 0) F.unit else F.raiseError(ObjectWithPropertyNotFound(name, value))
    }
