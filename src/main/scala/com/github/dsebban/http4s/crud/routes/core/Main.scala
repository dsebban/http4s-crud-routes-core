...
object Main extends IOApp {
  ...
  override def run(args: List[String]): IO[ExitCode] = {
    ...
    Stream
      .resource(MongoHelpers.getDB[IO]("mongodb://localhost:27017", dbName))
      // .eval(KVStore.createInMemory[IO, User, AuthInfo])
      .flatMap {
        // store =>
        db =>
          val store = KVStore.create[IO, User, AuthInfo](db, _.organization)
          val usersRepo: algebra.AuthedResourceAlgebra[IO, User, AuthInfo, MongoError] = interpreter
            .toAuthResourceAlgebra[IO, User, AuthInfo, MongoError](store, _ => BSONObjectID.generate().stringify)
          ...
          val routes: algebra.AuthedResourceAlgebra[IO, User, AuthInfo, MongoError] => HttpRoutes[IO] = users =>
