...
//   }
  ...
  val routes: HttpRoutes[F] = H.handle(getAll <+> getByID <+> create)
...
class UserAuthedRoutesMTL[F[_], R, E <: Throwable, A](resourceAlgebra: AuthedResourceAlgebra[F, R, A],
  ...
  // def combineErrors: NonEmptyList[E] => E =
  //   errors =>
  //     (errors.head, errors.tail) match {
  //       case (h, Nil)        => h
  //       case (h, e1 :: rest) => CompositeException(h, e1, rest)
  //   }
...
//   }
  ...
  val routes: HttpRoutes[F] = H.handle(Middleware(getAll <+> getByID <+> create))
